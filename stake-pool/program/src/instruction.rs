...
pub enum StakePoolInstruction {
    ...
    ///
    ///   The stake account must have the rent-exempt amount plus at least 1 PANO,
    ///   and at most 1.001 PANO.
    ///
    ///   Once we delegate even 1 PANO, it will accrue rewards one epoch later,
    ///   so we'll have more than 1 active PANO at this point.
    ///   At 10% annualized rewards, 1 epoch of 2 days will accrue
    ///   0.000547945 PANO, so we check that it is at least 1 PANO, and at most
    ///   1.001 PANO.
    ///
    ///   0. `[w]` Stake pool
    ///   1. `[s]` Staker
    ...
    ///
    ///   Only succeeds if the validator stake account has the minimum of 1 PANO
    ...
    /// The minimum amount to move is rent-exemption plus 1 PANO in order to avoid
    ...
    /// between PANO staked on different validators, the staker can force all
    ...
    ///
    ///   Succeeds if the stake account has enough PANO to cover the desired amount
    ///   of pool tokens, and if the withdrawal keeps the total staked amount
    ///   above the minimum of rent-exempt amount + 1 PANO.
    ///
