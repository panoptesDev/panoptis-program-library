...
async fn test_success() {
    ...
    // 100 SAFE * 80% LTV -> 80 SAFE * 20 USDC -> 1600 USDC borrow
    ...
    // 800 USDC / 20 USDC per SAFE -> 40 SAFE + 10% bonus -> 44 SAFE
    const SAFE_LIQUIDATION_AMOUNT_LAMPORTS: u64 = 44 * LAMPORTS_TO_SAFE * INITIAL_COLLATERAL_RATIO;
    ...
    assert_eq!(
        ...
        initial_user_collateral_balance + SAFE_LIQUIDATION_AMOUNT_LAMPORTS
    );
    ...
    assert_eq!(
        ...
        initial_collateral_supply_balance - SAFE_LIQUIDATION_AMOUNT_LAMPORTS
    );

    let obligation = test_obligation.get_state(&mut banks_client).await;
    assert_eq!(
        obligation.deposits[0].deposited_amount,
        SAFE_DEPOSIT_AMOUNT_LAMPORTS - SAFE_LIQUIDATION_AMOUNT_LAMPORTS
    );
    assert_eq!(
