...
async fn test_success() {
    ...
    const PANO_DEPOSIT_AMOUNT_LAMPORTS: u64 = 10 * LAMPORTS_TO_PANO * INITIAL_COLLATERAL_RATIO;
    ...
    let sol_test_reserve = add_reserve(
        ...
        &user_accounts_owner,
        AddReserveArgs {
            user_liquidity_amount: PANO_RESERVE_COLLATERAL_LAMPORTS,
            liquidity_amount: PANO_RESERVE_COLLATERAL_LAMPORTS,
    ...
    assert_eq!(
        ...
        initial_collateral_supply_balance + PANO_DEPOSIT_AMOUNT_LAMPORTS
    );
    let user_collateral_balance =
        get_token_balance(&mut banks_client, sol_test_reserve.user_collateral_pubkey).await;
    assert_eq!(
        user_collateral_balance,
        initial_user_collateral_balance - PANO_DEPOSIT_AMOUNT_LAMPORTS
    );
}
